...
export default function NameIndex({ names, filters }: Props) {
    const [search, setSearch] = useState(filters.search || '');

    const updateDataTable = (newParams: object) => {
        router.get('/admin/name',
            { ...filters, ...newParams },
            { preserveState: true, replace: true, only: ['names', 'filters'] }
        );
    };
    ...
    const handleDelete = (id: number) => {
        if (confirm('Apakah Anda yakin ingin menghapus nama ini?')) {
            router.delete(`/admin/name/${id}/delete`);
        }
    };
        ...
        <AppLayout>
            ...
            <div className="py-12 px-4 sm:px-6 lg:px-8">
                <div className="sm:flex sm:items-center justify-between mb-8">
                    <div>
                        <h1 className="text-2xl font-bold text-foreground">Master Names</h1>
                ...
                <div className="overflow-x-auto rounded-lg border border-border bg-card shadow-sm">
                    <table className="min-w-full divide-y divide-border">
                        ...
                        <tbody className="divide-y divide-border bg-card text-sm">
                            {names.data.map((name) => (
                                <tr key={name.id} className="hover:bg-muted/20 transition-colors">
                                    <td className="px-6 py-4 font-semibold">{name.name}</td>
                                    <td className="px-6 py-4 text-right flex justify-end gap-3">
                                        <Link href={`/admin/name/${name.id}/edit`} className="text-forest-600 hover:text-forest-800">
                                            <PencilSquareIcon className="w-5 h-5" />
                                        </Link>
                                        <button onClick={() => handleDelete(name.id)} className="text-red-500 hover:text-red-700">
                ...
                <div className="mt-6 flex items-center justify-between">
                    <p className="text-sm text-muted-foreground">Showing {names.from} to {names.to} of {names.total} results</p>
                    <div className="flex gap-1">
                        {names.links.map((link, i) => (
                            <button
                                key={i}
